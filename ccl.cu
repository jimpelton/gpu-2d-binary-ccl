...
bool cclSolveCCLShared(sCudaBuffer2D outLabelsBuf, sCudaBuffer2D inSegBuf, int threadsX, int threadsY, int imgWidth, int imgHeight, int dataWidth, int log2DataWidth)
{
	...
	solveCCLSharedKernel<<<grid, block, labelsShareSize+segShareSize>>>
		                                       ((int*)outLabelsBuf.m_data, outLabelsBuf.m_pitch/sizeof(int), 
											    ...
											    dataWidth, labelsShareSize/sizeof(int));
	return true;
}
...
bool cclMergeEquivalenceTrees(sCudaBuffer2D inOutLabelsBuf, sCudaBuffer2D inSegBuf, int* dChanged, int threadsX, int threadsY, int imgWidth, int imgHeight, int dataWidth, int log2DataWidth, int tileSize)
{
	cudaError_t err;
