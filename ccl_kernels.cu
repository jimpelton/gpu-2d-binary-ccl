...
void solveCCLSharedKernel(		 int* dLabelsOut, const int pitch,			
								 ...
								 const int dataWidth, const int sMemSegOff)
{	
		...
		if(threadIdx.y < blockDim.y-1) {
			...
			for(int i=0;i<threadIterations;++i) {					
				...
				if(seg != 0) {		
					...
					if(offset>0) unionF(dLabelsInOut, seg, tex2D(segmentedTex, x-1, y+1), idx, idx-1+pitch, sChanged);
					unionF(dLabelsInOut, seg, tex2D(segmentedTex, x, y+1), idx, idx+pitch, sChanged);
					if(offset<nextTileDim-1) unionF(dLabelsInOut, seg, tex2D(segmentedTex, x+1, y+1), idx, idx+1+pitch, sChanged);
